...
> ⚠️ **Página histórica.** Retrata SoundMate **antes de que existiera la capa de aplicación**:
> había modelo de dominio, persistencia y repositorios, pero **ni un solo endpoint de negocio**.
> Sigue siendo la mejor explicación de *qué son* las entidades y cómo se relacionan, y por eso se
> conserva. Para saber **qué hace SoundMate hoy**, la buena es [**v0.2.0**](v0.2.0.md).
>
> Lo que ya no cuadra: dice *129 tests* (hoy son 387) y lista como pendiente lo que la v0.2.0 ya
> resolvió — los casos de uso de usuarios y academias y sus endpoints. Tampoco menciona el borrado
> lógico, Docker ni la conexión con Agendia.

Resume **qué es SoundMate**, su **modelo de datos** (las entidades) y **cómo está construido**. Está en dos niveles: una parte **funcional** (para cualquiera) y otra **técnica** (para desarrollo).
